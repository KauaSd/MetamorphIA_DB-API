...
mensagem_al = Table(
    ...
    Column(
        "id_mensagem", Integer, ForeignKey("mensagens.id_mensagem"), primary_key=True
    ),
)
...
class alunos(base):
    ...
    mensagens_rel = relationship(
        "mensagens", secondary=mensagem_al, back_populates="alunos_rel"
    )
...
class mensagens(base):
    ...
    alunos_rel = relationship(
        "alunos", secondary=mensagem_al, back_populates="mensagens_rel"
    )
...
class user2famethods(base):
    ...
    id_prof = Column(Integer, ForeignKey(professores.id_prof))
class user2facodes(base):
    __tablename__ = "user_2fa_codes"
