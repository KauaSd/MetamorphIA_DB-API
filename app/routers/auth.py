...
async def logarProfessor( request: Request, dados: Annotated[LoginForm, Depends()], sessao: Annotated[Session, Depends(pegar_bd)]) -> Token:
    try:
    ...
    access_token = security.criar_access_token(data= {"sub": usuario.nome_prof}, expires_delta=access_token_expires )
